...
def troubleshoot_bluetooth():
    ...
    reset = "\033[0m"
    try:
        subprocess.run(['termux-bluetooth', 'status'], check=True, stdout=subprocess.PIPE)
    except subprocess.CalledProcessError:
        print(f"{reset}[{red}!{reset}] {red}CRITICAL{reset}: {blue}termux-bluetooth {reset}is not installed or not working properly.")
        return False
